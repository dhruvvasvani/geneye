...
# Load cascade classifiers once at module import time
FACE_CASCADE = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
EYE_CASCADE  = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_eye.xml")

# Warm up cascades — prevents a ~50 ms first-call delay
...
def extract_eye_region(image_np: np.ndarray, target_size: int = 384) -> Image.Image:
    """
    Given a BGR numpy image, detect the largest eye and return a
    square PIL image of size target_size.  Falls back gracefully if no
    face / eye is detected.
    """
    h, w = image_np.shape[:2]

    # ── 1. Detect face on a down-scaled image for speed ──────────────────────
    FACE_DET_DIM = 320
    ...
    if len(faces) == 0:
        # No face found — use top-half of image as fallback ROI
        ...
        # ── 2. Detect eye inside face ROI at reduced resolution ───────────
        EYE_DET_DIM = 256
    ...
    # Safety guard against degenerate crops
    if eye_crop.size == 0:
        eye_crop = image_np
